...
const ImportChiaHang: React.FC = () => {
  ...
  const [rows, setRows] = useState<ChiaHangRow[]>([]);
  const [preview, setPreview] = useState<PreviewResult<ChiaHangRow> | null>(null);
  ...
  const onDrop = useCallback(async (acceptedFiles: File[]) => {
    ...
    try {
      const parsedRows = await parseExcelToChiaHang(selectedFile);
      setRows(parsedRows);
  ...
  const handlePreview = async () => {
    ...
    setLoading(true);
    try {
      const result = await previewChiaHang(rows, thang, nam);
      setPreview(result);
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      const result = await confirmChiaHang(preview.hopLe, file.name, fileHash);
    ...
    <div className="space-y-6">
                  ...
                  <tbody className="divide-y divide-gray-200">
                    {preview.hopLe.slice(0, 100).map((row, idx) => (
                      <tr key={idx} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 text-sm text-right">{row.sanPhamDat}</td>
                        <td className="px-4 py-2 text-sm text-right">{row.sanPhamLoi}</td>
                        <td className="px-4 py-2 text-sm text-right">{row.tongSanPham}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
      ...
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        ...
        <ul className="text-sm text-yellow-700 space-y-1 list-disc list-inside">
          <li>File Excel cần có các cột: Mã NV, Ngày, SP Đạt, SP Lỗi, Tổng SP</li>
          <li>Ngày có thể ở định dạng dd/MM/yyyy hoặc yyyy-MM-dd</li>
          <li>Nếu dữ liệu đã tồn tại (cùng mã NV + ngày) sẽ được cập nhật</li>
          <li>
            Công thức: <strong>Quy đổi SP = SP Đạt - (SP Lỗi × 5)</strong>, <strong>Tiền = Quy đổi × 320đ</strong>
          </li>
        </ul>
      </div>
    </div>
