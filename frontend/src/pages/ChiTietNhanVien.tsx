...
import NhanVienThongTinTab from '../components/nhan-vien/NhanVienThongTinTab'
import { NhanVienDetailForm } from '../types/nhanVienDetail'

type TabType = 'thong-tin' | 'hop-dong' | 'phu-cap' | 'lich-su' | 'lich-su-phong-ban'

const createEmptyNhanVienForm = (): NhanVienDetailForm => ({
  hoTen: '',
  email: '',
  soDienThoai: '',
  phongBanId: 0,
...
const mapNhanVienToForm = (nhanVien: any): NhanVienDetailForm => ({
  ...
  hinhCCCDTruoc: nhanVien?.hinhCCCDTruoc || '',
  hinhCCCDSau: nhanVien?.hinhCCCDSau || '',
  soDienThoaiKhanCap: nhanVien?.soDienThoaiKhanCap || '',
  nguoiLienHeKhanCap: nhanVien?.nguoiLienHeKhanCap || '',
  quanHeKhanCap: nhanVien?.quanHeKhanCap || '',
  dongBHXH: nhanVien?.dongBHXH !== false,
})
...
export default function ChiTietNhanVien() {
  ...
  const [formNhanVien, setFormNhanVien] = useState<NhanVienDetailForm>(createEmptyNhanVienForm())
  ...
  useEffect(() => {
    if (nhanVien) {
      setFormNhanVien(mapNhanVienToForm(nhanVien))
    }
  }, [nhanVien])
    ...
    <div>
      ...
      {activeTab === 'thong-tin' && (
        <NhanVienThongTinTab
          ...
          setFormNhanVien={setFormNhanVien}
          phongBans={phongBans}
          tongPhuCap={tongPhuCap}
          onSave={handleCapNhatNhanVien}
          onCancel={handleHuyChinhSua}
          isSaving={capNhatNhanVienMutation.isPending}
        />
      )}
