...
let ReportsService = ReportsService_1 = class ReportsService {
    ...
    async getOT(filter) {
        ...
        const tongTienOT = bangLuongs.reduce((s, bl) => s + bl.chiTiets.reduce((ss, ct) => ss + Number(ct.soTien || 0), 0), 0);
        const otByPB = new Map();
        chiTiet.forEach(c => {
    ...
    async getQuyLuong(filter) {
        ...
        const tongBHXH_NLD = bangLuongs.reduce((s, bl) => s + bl.chiTiets
            .filter(ct => ct.khoanLuong?.maKhoan?.includes('BHXH_NLD'))
            .reduce((ss, ct) => ss + Number(ct.soTien || 0), 0), 0);
        const tongBHXH_DN = bangLuongs.reduce((s, bl) => s + bl.chiTiets
            .filter(ct => ct.khoanLuong?.maKhoan?.includes('BHXH_DN'))
            .reduce((ss, ct) => ss + Number(ct.soTien || 0), 0), 0);
        const tongThueTNCN = bangLuongs.reduce((s, bl) => s + bl.chiTiets
    ...
    async getHeadcount(filter) {
        ...
        const theoPhongBan = phongBans.map(pb => {
            ...
            const moiTuyen = nvs.filter(nv => nv.ngayVaoLam && nv.ngayVaoLam >= ngayDau && nv.ngayVaoLam <= ngayCuoi).length;
            const nghiMoi = nvs.filter(nv => nv.ngayNghiViec && nv.ngayNghiViec >= ngayDau && nv.ngayNghiViec <= ngayCuoi).length;
            return {
                phongBanId: pb.id,
