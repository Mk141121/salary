...
VALUES ('${ca.maCa}', '${ca.tenCa}', '${ca.gioVao}', '${ca.gioRa}', ${gioNghiTrua}, ${hetNghiTrua}, true, NOW())
...
sql.push(`
-- Tạo tài khoản cho nhân viên đang làm (password: 123456)
...
SELECT 
  ...
  nv.id,
  CASE 
    WHEN pb.ma_phong_ban IN ('KV', 'CH') THEN (SELECT id FROM ca_lam_viec WHERE ma_ca = 'CA_KHO' LIMIT 1)
    WHEN pb.ma_phong_ban IN ('SHIP', 'GIAO_HANG') THEN (SELECT id FROM ca_lam_viec WHERE ma_ca = 'CA_GIAO_HANG' LIMIT 1)
    ELSE (SELECT id FROM ca_lam_viec WHERE ma_ca = 'CA_HANH_CHINH' LIMIT 1)
  END,
  d::date,
  NOW()
FROM nhan_vien nv
...
// ==========================================
sql.push('-- 9. TẠO DỮ LIỆU CHẤM CÔNG MẪU');
sql.push(`
...
-- Tạo chi tiết chấm công cho 2 tuần đầu (02-17/01/2026)
...
SELECT 
  ...
  CASE 
    WHEN random() < 0.05 THEN 'VANG_MAT'
    WHEN random() < 0.15 THEN 'DI_TRE'
    ELSE 'DU_CONG'
  END,
  CASE WHEN random() < 0.15 THEN floor(random() * 30 + 5)::int ELSE 0 END,
  CASE WHEN random() < 0.05 THEN floor(random() * 20)::int ELSE 0 END,
  8.0,
  NOW()
FROM nhan_vien nv
JOIN phong_ban pb ON nv.phong_ban_id = pb.id
...
// 12. BẢNG LƯƠNG MẪU
// ==========================================
sql.push('-- 12. TẠO BẢNG LƯƠNG MẪU');
sql.push(`
...
SELECT 
  ...
  22,
  26,
  nv.luong_co_ban + 500000,
  nv.luong_co_ban * 0.105,
  nv.luong_co_ban + 500000 - nv.luong_co_ban * 0.105,
  NOW()
FROM nhan_vien nv
...
SELECT 
  ...
  'Hệ thống quản lý nhân sự HRM-Lite phiên bản 2 đã sẵn sàng!',
  'HE_THONG',
  false,
  NOW()
FROM nguoi_dung nd
