    ...
    FOR v_thang IN 6..11 LOOP
        ...
        FOR v_pb_id IN SELECT id FROM phong_ban WHERE trang_thai = 'HOAT_DONG' LOOP
            ...
            FOR v_nv IN SELECT id, ma_nhan_vien, ho_ten FROM nhan_vien WHERE phong_ban_id = v_pb_id AND trang_thai = 'DANG_LAM' LOOP
                ...
                IF RANDOM() > 0.4 THEN
                    INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                    VALUES (v_bl_id, v_nv.id, 22, 500000 + (RANDOM() * 2000000)::INT, 'CO_DINH', NOW(), NOW());
                END IF;
                
                -- Thưởng doanh số (23) - Kinh doanh
                IF v_pb_id = 29 THEN
                    INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                    VALUES (v_bl_id, v_nv.id, 23, 1000000 + (RANDOM() * 5000000)::INT, 'CO_DINH', NOW(), NOW());
                END IF;
                
                -- BHXH 8% (24)
                INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                VALUES (v_bl_id, v_nv.id, 24, v_luong_cb * 0.08, 'CO_DINH', NOW(), NOW());
                
                -- BHYT 1.5% (25)
                INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                VALUES (v_bl_id, v_nv.id, 25, v_luong_cb * 0.015, 'CO_DINH', NOW(), NOW());
                
                -- BHTN 1% (26)
                INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                VALUES (v_bl_id, v_nv.id, 26, v_luong_cb * 0.01, 'CO_DINH', NOW(), NOW());
                
                -- Thuế TNCN (27) - nếu lương > 11tr
                IF v_luong_cb > 11000000 THEN
                    INSERT INTO chi_tiet_bang_luong (bang_luong_id, nhan_vien_id, khoan_luong_id, so_tien, nguon, ngay_tao, ngay_cap_nhat)
                    VALUES (v_bl_id, v_nv.id, 27, (v_luong_cb - 11000000) * 0.05, 'CO_DINH', NOW(), NOW());
                END IF;
                ...
                VALUES (v_bl_id, v_nv.id, v_so_ngay, (RANDOM() * 2)::INT, CASE WHEN RANDOM() < 0.1 THEN 1 ELSE 0 END, NOW(), NOW());
...
SELECT 'Snapshot' as loai, COUNT(*) as so_dong FROM snapshot_bang_luong;
SELECT 'Sự kiện' as loai, loai_su_kien, COUNT(*) FROM su_kien_thuong_phat WHERE ngay >= '2025-06-01' GROUP BY loai_su_kien;
SELECT 'Sản lượng chia hàng' as loai, COUNT(*) FROM snapshot_san_luong_chia_hang;
SELECT 'Sản lượng giao hàng' as loai, COUNT(*) FROM snapshot_giao_hang;
